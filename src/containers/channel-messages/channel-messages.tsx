...
import { setOrder } from "../../features/message/message-slice";
...
function ChannelMessages({ closeAnnouncement }: ChannelMessagesProps) {
  ...
  const rows: TableRow<Message>[] = messages.map((m) => ({
    data: m,
                      ...
                      renderOption={(params, id) => {
                        ...
                        return (
                          <Typography gap="4px" {...params}>
                            {foundGuild && <GuildIcon guild={foundGuild} />}
                            {foundGuild?.name}
                          </Typography>
                        );
                      }}
          ...
          <>
            {messages.length > 0 && (
              <Box sx={{ maxHeight: "430px", overflow: "auto" }}>
                {Table<Message>({ columns, rows, orderProps })}
              </Box>
            )}
